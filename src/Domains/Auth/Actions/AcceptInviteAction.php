...
class AcceptInviteAction extends Action
{
    public function run($email, $code) {
        //validate request
        $validation = ClassName::Validator('AcceptInviteValidator')::run($data);
        if ( $validation !== true ) {
            return Response::error($validation);
        }

        $data = ClassName::Transformer('AcceptInviteTransformer')::run($data);
        ...
        //generate jwt token
        $item = ClassName::Presenter('Presenter')::run(\Auth::user(), $invite->team_id);
